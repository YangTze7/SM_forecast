...
def LoadConstantMask(batch_size):   # TODO: Replace dummy masks with actual data
    ...
    land_mask = torch.ones((batch_size, 1, 128, 128))
    soil_type = torch.ones((batch_size, 1, 128, 128))
    topography = torch.ones((batch_size, 1, 128, 128))
...
class WeatherModel(nn.Module):
    ...
    # n_heads = [6, 12, 12, 6]
    # D = 4
    def __init__(self, C=192, depth=[2, 6, 6, 2], n_heads=[6, 12, 12, 6], D=4, batch_size=4, log_GPU_mem=False, **kwargs):
        C = 192
        super().__init__()
        ...
        # Four main layers:
        self.layer1 = EarthSpecificLayer(depth=depth[0], dim=C, input_resolution=(4, 32, 32),
                                        heads=n_heads[0], drop_path_ratio_list=drop_path_list[:2], D=D)
        #self.middleLayers = nn.Sequential(
        self.layer2 = EarthSpecificLayer(depth=depth[1], dim=2*C, input_resolution=(4, 16, 16), heads=n_heads[1], drop_path_ratio_list=drop_path_list[2:], D=D)
        self.layer3 = EarthSpecificLayer(depth=depth[2], dim=2*C, input_resolution=(4, 16, 16), heads=n_heads[2], drop_path_ratio_list=drop_path_list[2:], D=D)
        #)
        self.layer4 = EarthSpecificLayer(depth=depth[3], dim=C, input_resolution=(4, 32, 32),
    ...
    def forward(self, data1,data2):
        ...
        #   ->   (B, Z=8, H=360, W=181, C)
        x = self.input_layer(data1, data2)
        ...
        output_surface = self.output_layer(x)
...
class PatchEmbedding(nn.Module):
    def __init__(self, patch_size, dim, batch_size): # patch_size = (2,4,4)
        ...
        self.conv_surface = nn.Conv2d(in_channels=5, out_channels=dim, kernel_size=patch_size[1:], stride=patch_size[1:])
    ...
    def forward(self, input_air, input_surface):
        ...
        # input_air = F.pad(input_air, (0,0,0,0,1,0))
        ...
        # land_mask = torch.ones((input_surface.shape[0], 1, 128, 128))
        # soil_type = torch.ones((input_surface.shape[0], 1, 128, 128))
        # topography = torch.ones((input_surface.shape[0], 1, 128, 128))
        # input_surface = torch.cat((input_surface, land_mask, soil_type,topography), 1)
...
class PatchRecovery(nn.Module):
    def __init__(self, patch_size, dim):
        ...
        self.tconv_surface = nn.ConvTranspose2d(dim, 5, patch_size[1:], patch_size[1:])
        self.tconv_out = nn.Conv3d(16, 5, (1, 1, 1))

    def forward(self, x):

        # x = torch.permute(x, (0,4,1,2,3))  
        # output_surface = self.tconv_air(x[:, :, :, :, :])
        # output_surface = output_surface.permute(0, 2,1, 3,4)
        # output_surface = self.tconv_out(output_surface)
        # output_surface = output_surface.permute(0, 2,1, 3,4)
...
class EarthAttention3D(nn.Module):
    def __init__(self, dim, heads, window_size, input_resolution, dropout_rate):
        ...
        self.input_resolution = (input_resolution[0], input_resolution[1], input_resolution[2])        # shape: (8, 360, 181) or (8, 180, 91) (+5 padding to W dim)
...
if __name__ == "__main__":
    ...
    air_input = torch.randn((1,13,128,128,10))
    surface_input = torch.randn((1,128,128,4))
    input = [air_input,surface_input]
