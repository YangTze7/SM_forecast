...
from openstl.models import WeatherModel
...
class PANGU(Base_method):
    ...
    def _build_model(self, args):
        return WeatherModel(**args).to(self.device)



    def _predict(self, batch_x, sur,batch_y=None, **kwargs):
        """Forward the model"""
        if self.args.aft_seq_length == self.args.pre_seq_length:
            pred_y = self.model(batch_x,sur)
            ...
            # dem = batch_x[:,:,15:16,:,:]
            for _ in range(d):
                cur_seq,sur_seq = self.model(cur_seq,sur_seq)
                # cur_seq = torch.concat([cur_seq,dem],dim=2)
                pred_y.append(sur_seq.unsqueeze(1))
                sur_seq = sur_seq.permute(0, 2, 3, 1)

            if m != 0:
                cur_seq,sur_seq = self.model(cur_seq,sur_seq)
                # cur_seq = torch.concat([cur_seq,dem],dim=2)
                pred_y.append(sur_seq[:, :m].unsqueeze(1))
                sur_seq = sur_seq.permute(0, 2, 3, 1)
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, sur,batch_y in train_pbar:
            ...
            if not self.args.use_prefetcher:
                batch_x, sur,batch_y = batch_x.to(self.device),sur.to(self.device), batch_y.to(self.device)
            runner.call_hook('before_train_iter')
            
            # with self.amp_autocast():
            # input_x = [batch_x,sur]
            pred_y = self._predict(batch_x,sur)
            # pred_y = pred_y.unsqueeze(1)
            pred_y = pred_y+sur.permute(0,3,1,2)
            batch_y = batch_y[:,0]+sur.permute(0,3,1,2)
            # print(pred_y.shape)
