...
from openstl.utils.img_utils import PeriodicPad2d
...
class PrecipNet(nn.Module):
    def __init__(self, backbone):
        ...
        self.conv = nn.Conv2d(self.in_chans*20, self.out_chans*20, kernel_size=3, stride=1, padding=0, bias=True)
        self.act = nn.ReLU()
...
class AFNO_Model(nn.Module):
    def __init__(
        ...
        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=self.patch_size, in_chans=in_chans*self.T, embed_dim=embed_dim)
        ...
        self.head = nn.Linear(embed_dim, self.out_chans*self.T*self.patch_size[0]*self.patch_size[1], bias=False)
    ...
    def forward(self, x):
        ...
        # c_out = int(self.embed_dim/20)
        # c_out = 7
        # x = x.reshape(x.shape[0],20,c_out,x.shape[-2],x.shape[-1])
        x = x.reshape(x.shape[0],self.T, self.in_chans, x.shape[-2],x.shape[-1])
        return x
...
if __name__ == "__main__":
    # params = YParams("config/AFNO.yaml", "full_field")
    model = AFNO_Model(img_size=(128, 256), T=2,patch_size=16, in_chans=7, out_chans=7)
    sample = torch.randn(1, 2,7, 128, 256)
    result = model(sample)
    print(result.shape)
