...
class BaseExperiment(object):
    ...
    def train(self):
        ...
        for epoch in range(self._epoch, self._max_epochs):
            ...
            if epoch % self.args.log_step == 0:
                ...
                with torch.no_grad():
                    vali_loss,vali_loss1,vali_loss2, vali_loss3, vali_loss4, vali_loss5 = self.vali()
                ...
                if self._rank == 0:
                    ...
                    print_log('Vali Loss1: {0:.7f}, Loss2: {1:.7f} | Lr: {2:.7f} | Loss3: {3:.7f} | Loss4: {4:.7f}\n'.format(
                        vali_loss1, vali_loss2, vali_loss3, vali_loss4, vali_loss5))
    ...
    def vali(self):
        ...
        return results['loss'].mean(),results['loss1'].mean(),results['loss2'].mean(),results['loss3'].mean(),results['loss4'].mean(),results['loss5'].mean()
    ...
    def infer(self):
        ...
        c=4
        if self.args.test:
            self.path = "/home/convlstm_predict/OpenSTL-master/work_dirs/era5_tau_t2m"
            best_model_path = osp.join(self.path, 'checkpoint069.pth')

            self._load_from_state_dict(torch.load(best_model_path))


        data_dir = '/home/convlstm_predict/2023tianchi_weather_prediction/weather_round1_test/input' # change to you dataset dir
        save_dir = '/home/convlstm_predict/2023tianchi_weather_prediction/weather_round1_test/output/tp'
        step  = 1

        level = 1
        from openstl.datasets.dataloader_era5_infer import load_data
        dataloader_test = \
        ...
        self.call_hook('before_val_epoch')
        results = self.method.infer_one_epoch(self, dataloader_test,recurrent=False)
        self.call_hook('after_val_epoch')


        for i in range(len(results)):
            pred = results[i][0][:,c:c+1]
            # pred = pred[:,x,:,:]
