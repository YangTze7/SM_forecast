...
class Base_method(object):
    ...
    def _nondist_forward_collect(self, data_loader, length=None, gather_data=False):
        ...
        for idx, (batch_x, batch_y) in enumerate(data_loader):
            ...
            if gather_data:  # return raw datas
                ...
                # eval_res['loss'] = self.criterion(pred_y, batch_y).cpu().numpy()
                eval_res['loss1'] = self.criterion(pred_y[:,:,0], batch_y[:,:,0]).cpu().numpy()
                eval_res['loss2'] = self.criterion(pred_y[:,:,1], batch_y[:,:,1]).cpu().numpy()
                eval_res['loss3'] = self.criterion(pred_y[:,:,2], batch_y[:,:,2]).cpu().numpy()
                eval_res['loss4'] = self.criterion(pred_y[:,:,3], batch_y[:,:,3]).cpu().numpy()
                eval_res['loss5'] = self.criterion(pred_y[:,:,4], batch_y[:,:,4]).cpu().numpy()
                eval_res['loss'] = (eval_res['loss1']+eval_res['loss2']+eval_res['loss3']+eval_res['loss4']+eval_res['loss5'])/5
                for k in eval_res.keys():
                    eval_res[k] = eval_res[k].reshape(1)
                results.append(eval_res)
    ...
    def _nondist_forward_infer(self, data_loader,recurrent, length=None, gather_data=False):
        """Forward and collect predictios.
        ...
        for idx, batch_x in enumerate(data_loader):
            with torch.no_grad():
                ...
                batch_x=batch_x.float()
                if recurrent:
                    pred_y = self._predict(batch_x,batch_x)
                else:
    ...
    def infer_one_epoch(self, runner, test_loader,recurrent, **kwargs):
        """Evaluate the model with test_loader.
        ...
        results = self._nondist_forward_infer(test_loader,recurrent, gather_data=True)
