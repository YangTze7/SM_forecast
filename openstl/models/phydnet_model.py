...
class PhyDNet_Model(nn.Module):
    ...
    def __init__(self, configs, **kwargs):
        ...
        self.encoder1 = PhyD_EncoderRNN(self.phycell, self.convcell,
                                       in_channel=1, patch_size=patch_size)
    ...
    def forward(self, input_tensor, target_tensor, constraints, teacher_forcing_ratio=0.0):
        ...
        for ei in range(self.pre_seq_length - 1):
            ...
            loss += self.criterion(output_image, input_tensor[:,ei+1,0:1,:,:])
        ...
        for di in range(self.aft_seq_length):
            if di == 0:
                ...
                _, _, output_image, _, _ = self.encoder1(decoder_input)
                target = target_tensor[:,di,:,:,:]
            loss += self.criterion(output_image, target)
        ...
        with torch.no_grad():
            ...
            for di in range(self.aft_seq_length):
                if di==0:
                    ...
                    _, _, output_image, _, _ = self.encoder1(decoder_input, False, False)
