...
def load_data(dataname, batch_size, val_batch_size, num_workers, data_root, dist=False, **kwargs):
    ...
    if dataname == "gldas":
        from .dataloader_gldas import load_data
        return load_data(batch_size, val_batch_size, data_root, **cfg_dataloader)
    if dataname == "era5":
        from .dataloader_era5 import load_data
        return load_data(batch_size, val_batch_size, data_root, **cfg_dataloader)
    if dataname == 'floodx':
        from .dataloader_floodx import load_data
        return load_data(batch_size, val_batch_size, data_root, event=kwargs.get('event', None),**cfg_dataloader)
    if dataname == 'flood':
        from .dataloader_flood import load_data
        return load_data(batch_size, val_batch_size, data_root, **cfg_dataloader)
