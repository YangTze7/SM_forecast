...
class PredRNN_Model(nn.Module):
    ...
    def __init__(self, num_layers, num_hidden, configs, **kwargs):
        ...
        for i in range(num_layers):
            in_channel = int(self.frame_channel/10) if i == 0 else num_hidden[i - 1]
            cell_list.append(
                SpatioTemporalLSTMCell(in_channel, num_hidden[i], height, width,
                                       configs.filter_size, configs.stride, configs.layer_norm))
        self.cell_list = nn.ModuleList(cell_list)
        self.conv_first = nn.Conv2d(self.frame_channel, int(self.frame_channel/10),
                                   kernel_size=1, stride=1, padding=0, bias=False)
        self.conv_last = nn.Conv2d(num_hidden[num_layers - 1], int(self.frame_channel/10),
                                   kernel_size=1, stride=1, padding=0, bias=False)

    def forward(self, frames_tensor, frames_tensor_gt,mask_true, **kwargs):
        ...
        if kwargs.get('return_loss', True):
            loss = self.MSE_criterion(next_frames[:, 1:], frames_tensor_gt)
        else:
            loss = None
