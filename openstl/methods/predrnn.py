...
class PredRNN(Base_method):
    ...
    def _predict(self, batch_x, batch_y, **kwargs):
        ...
        # preprocess
        # batch_x = batch_x.reshape(-1, batch_y.shape[1], batch_y.shape[2], 256, 256)
        batch_x = batch_x.permute(0, 1, 3, 4, 2).contiguous()
        batch_x = reshape_patch(batch_x, self.args.patch_size)

        batch_y = batch_y.permute(0, 1, 3, 4, 2).contiguous()
        batch_y = reshape_patch(batch_y, self.args.patch_size)
        # test_ims = torch.cat([batch_x, batch_y], dim=1).permute(0, 1, 3, 4, 2).contiguous()
        # test_dat = reshape_patch(batch_x, self.args.patch_size)
        ...
        img_gen, _ = self.model(batch_x, batch_y,real_input_flag, return_loss=False)
        ...
        for batch_x, batch_y in train_pbar:
            ...
            if self.args.reverse_scheduled_sampling == 1:
                real_input_flag = reserve_schedule_sampling_exp(
                    num_updates, batch_x.shape[0], self.args)
            else:
                eta, real_input_flag = schedule_sampling(
                    eta, num_updates, batch_x.shape[0], self.args)

            with self.amp_autocast():
                img_gen, loss = self.model(batch_x, batch_y,real_input_flag)
